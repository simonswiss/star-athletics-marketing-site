...
import { DRAFT_BRANCH_PREFIX } from '../src/lib/publishing/config'
...
test('installed CMS patch locks direct branch URLs and labels saved drafts', () => {
  ...
  assert.ok(ui.includes('data?.viewer?.databaseId'))
  assert.ok(ui.includes('draftIdentityReady: true'))
  assert.ok(ui.includes('const t21 = "Save draft"'))
})

test('draft branch never triggers automatic Vercel deployments', () => {
  const config = JSON.parse(readFileSync('vercel.json', 'utf8'))
  assert.equal(config.git.deploymentEnabled[`${DRAFT_BRANCH_PREFIX}*`], false)
  assert.equal(config.git.deploymentEnabled['content-drafts'], false)
})
